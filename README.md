...
El código está organizado como monolito modular por capacidades en `src/modules`: `identity-access`, `tenant-management`, `catalog`, `menu-customization`, `merchandising`, `public-menu` y `admin-dashboard`. Cada módulo publica solamente `contracts.ts`, `server.ts` y, cuando tiene interfaz, `ui.ts`; dominio, aplicación, infraestructura y presentación permanecen privados.
...
En una instalación existente, hacé un backup consistente de SQLite y de `STORAGE_ROOT`, ejecutá `bun run db:migrate-storage` durante la ventana de mantenimiento y reiniciá el servicio. El migrador aborta si encuentra archivos referenciados que no existen.
...
Las migraciones también pueden ejecutarse de forma independiente con `bash scripts/deploy/migrate.sh /ruta/al/release`; el script detiene temporalmente el servicio, respalda la base y verifica compatibilidad antes de reiniciarlo. Los seeds y el migrador de archivos legacy son operaciones manuales bloqueadas, no forman parte del deploy habitual:

```bash
bash scripts/deploy/run-locked.sh bun run db:seed
bash scripts/deploy/run-locked.sh bun run db:migrate-storage
```
