...
export function CanvasEditor({ project, initialAssets, restaurantName, restaurantSlug }: { project: MenuProjectView; initialAssets: MenuAssetView[]; restaurantName: string; restaurantSlug: string }) {
  ...
  const addIcon = () => addNode({ id: crypto.randomUUID(), type: "icon", iconKey: "star", ...newNodeFrame(80, 80), rotation: 0, opacity: 1, visible: true, locked: false, groupId: null, link: null, fill: "#B8790A", strokeWidth: 2 });
  ...
  return <>
    ...
    <div className="flex min-h-0 flex-1">
      <aside className="w-56 shrink-0 overflow-y-auto border-r border-zinc-200 bg-white p-3">
        <button type="button" className="mb-3 flex w-full items-center justify-between rounded-lg border border-zinc-200 bg-white px-2.5 py-2 text-left text-xs font-semibold text-zinc-700 transition hover:border-emerald-300 hover:bg-emerald-50" onClick={() => setLayersOpen((open) => !open)} aria-expanded={layersOpen} aria-controls="editor-layers"><span><Layers3 size={14} className="mr-1.5 inline text-emerald-700" />Capas</span><span className="text-[11px] font-normal text-zinc-400">{layersOpen ? "Ocultar" : "Mostrar"}</span></button>
