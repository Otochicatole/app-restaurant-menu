...
function OpacityControl({ node, onChange }: { node: CanvasNode; onChange: (opacity: number) => void }) {
  return <label className="mt-3 block text-xs">Opacidad <span className="float-right text-zinc-500">{Math.round(node.opacity * 100)}%</span><input aria-label="Opacidad del objeto" disabled={node.locked} className="mt-1 w-full accent-emerald-700 disabled:opacity-40" type="range" min="0" max="1" step="0.01" value={node.opacity} onChange={(event) => onChange(Number(event.target.value))} /></label>;
}
