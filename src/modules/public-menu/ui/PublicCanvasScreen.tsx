...
export function PublicCanvasScreen({ menu }: { menu: PublicCanvasMenuView }) {
  ...
  return <main className="relative h-[100dvh] w-full overflow-hidden bg-zinc-100">
    ...
    <section className="sr-only" aria-label="Contenido accesible de la carta">{textNodes.map((node) => node.link ? <a key={node.id} href={node.link}>{node.text}</a> : <p key={node.id}>{node.text}</p>)}{Object.values(menu.assets).filter((asset) => asset.kind === "IMAGE").map((asset) => <Image key={asset.id} src={asset.url} alt={asset.name} width={1} height={1} />)}</section>
    {accessible && <section className="absolute inset-x-3 bottom-16 z-20 max-h-[45dvh] overflow-y-auto rounded-xl bg-white p-4 shadow-xl" aria-label="Contenido accesible de la carta">{textNodes.length ? textNodes.map((node) => node.link ? <a key={node.id} href={node.link} className="mb-2 block text-sm text-zinc-800">{node.text}</a> : <p key={node.id} className="mb-2 text-sm text-zinc-800">{node.text}</p>) : <p className="text-sm text-zinc-500">Esta carta no tiene texto visible.</p>}</section>}
  </main>;
}
