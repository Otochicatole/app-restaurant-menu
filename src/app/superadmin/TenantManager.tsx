...
export function TenantManager({ tenants, createTenant, updateTenant, toggleTenant, resetPassword, deleteTenant }: {
  ...
  async function run(action: (formData: FormData) => Promise<Result>, formData: FormData, tenantId?: string) {
    setLoading(true); setNotice(null);
    const result = await action(formData);
    setLoading(false);
    if (!result.success) { setNotice(result.error ?? "No se pudo completar la operación"); return; }
    if (result.temporaryPassword) { setTemporaryPassword({ value: result.temporaryPassword, tenantId }); router.refresh(); }
    else window.location.reload();
  }

  return <div className="space-y-8">
    {notice && <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{notice}</div>}
    {temporaryPassword && !temporaryPassword.tenantId && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-950"><p className="font-bold">Contraseña temporal (guardala ahora)</p><code className="mt-3 block rounded-lg bg-white px-3 py-2 text-base">{temporaryPassword.value}</code><button onClick={() => setTemporaryPassword(null)} className="mt-3 text-xs font-bold uppercase tracking-wide">Cerrar</button></div>}
